...
# In-memory bootstrap cache (TTL 120 seconds)
...
async def get_bootstrap_data(
    ...
    # Serve from in-memory cache if fresh (< 120s)
    ...
    # Execute queries sequentially on the active, authenticated DB connection
    # This avoids PgBouncer connection pool contention, lock timeouts, and SSL handshakes
    try:
    ...
    try:
        ...
        c_res = await db.execute(c_query.order_by(Client.id.desc()).limit(150))
    ...
    try:
        ...
        a_res = await db.execute(a_query.order_by(Appointment.id.desc()).limit(150))
    ...
    try:
        ...
        i_res = await db.execute(i_query.order_by(InventoryItem.id.desc()))
    ...
    try:
        ...
        pkg_res = await db.execute(pkg_query.limit(100))
        packages = [safe_dump(p, ClientPackageResponse) for p in pkg_res.scalars().all()]
    except Exception:
        packages = []
    ...
    if role in ("admin", "partner"):
        ...
        try:
            ...
            t_res = await db.execute(t_query.order_by(FinancialTransaction.id.desc()).limit(150))
    ...
    _BOOTSTRAP_CACHE_EXPIRY[cache_key] = now + 120.0
