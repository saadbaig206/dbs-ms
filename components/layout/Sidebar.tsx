...
export const Sidebar: React.FC = () => {
  ...
  const tagline =
    clinicInfo.name.split(" ").slice(1).join(" ") || "Management System";
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-[#0F172A] text-slate-300 select-none border-r border-slate-800 overscroll-contain">
          ...
          {!isCollapsed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <h1
                ...
                className="text-base font-extrabold text-white tracking-wider uppercase font-sans"
              >
                {shortName}
              </h1>
              <p
                suppressHydrationWarning
                className="text-[10px] text-blue-400 font-semibold tracking-widest uppercase"
              >
                {tagline}
              </p>
            </motion.div>
      ...
      {/* Navigation List */}
      <div className="flex-1 py-4 px-3 space-y-1 overflow-y-auto overflow-x-hidden overscroll-contain custom-scrollbar">
        {menuItems
          .filter((item) => {
          ...
          .map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 shrink-0 transition-transform ${isActive ? "scale-110" : "group-hover:scale-105"}`}
                />
